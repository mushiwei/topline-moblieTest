<template>
  ...
  <div>
    ...
    <van-tabs class="channel-tabs" v-model="activeChannelIndex">
      <van-tab title="标签 1">
        <!-- 下拉刷新组件 isLoading 控制下拉的loading 状态 refresh 下拉之后触发的事件 -->
        <van-pull-refresh v-model="pullRefreshLoading" @refresh="onRefresh">
          <!-- loading 控制加载更多的loading状态 finished 控制是否已加载结束 onLoad事件会在滚动到底部区域的时候自动调用,每次onLoad 它会自动让 loading 为true-->
      ...
      <van-tab title="标签 5">内容 2</van-tab>
      <van-tab title="标签 6">内容 3</van-tab>
...
.channel-tans /deep/ .van-tabs_wrap {
  position: fixed;
  top: 92px;
}
